...
{
    SetMountCapability(MOUNT_CAN_PARK | MOUNT_CAN_ABORT | MOUNT_CAN_SYNC | MOUNT_CAN_GOTO |
                           MOUNT_HAS_TIME | MOUNT_HAS_LOCATION | MOUNT_HAS_PIER_SIDE, TEMMA_SLEW_RATES);
...
bool TemmaMount::initProperties()
{
    ...
    IUFillSwitchVector(&SlewRateSP, SlewRateS, 2, getDeviceName(), "MOUNT_SLEW_RATE", "Slew Rate", MOTION_TAB,
...
bool TemmaMount::SendCommand(const char *cmd, char *response)
{
    ...
    if (isSimulation())
    {
        ...
        case 'E':
        {
            ...
            char state = 'E';
            if (TrackState == MOUNT_PARKED || TrackState == MOUNT_IDLE || TrackState == MOUNT_TRACKING)
                state = 'F';
...
bool TemmaMount::ReadScopeStatus()
{
    ...
    if (IsAlignmentSubsystemActive())
    {
        ...
        INDI::AlignmentSubsystem::MountDirectionVector TDV = MountDirectionVectorFromLocalHourAngleDeclination(RaDec);
        //AlignmentSubsystem::MountDirectionVector TDV = MountDirectionVectorFromEquatorialCoordinates(RaDec);
        ...
        if (!TransformMountToCelestial(TDV, alignedRA, alignedDEC))
        {
            aligned = false;
            DEBUGF(INDI::AlignmentSubsystem::DBG_ALIGNMENT, "Failed TransformMountToCelestial: Scope RA=%g Scope DE=%f, Aligned RA=%f DE=%f", currentRA, currentDEC, alignedRA, alignedDEC);
        }
        else
        {
            DEBUGF(INDI::AlignmentSubsystem::DBG_ALIGNMENT, "TransformMountToCelestial: Scope RA=%f Scope DE=%f, Aligned RA=%f DE=%f", currentRA, currentDEC, alignedRA, alignedDEC);
        }
    }
...
bool TemmaMount::MoveNS(INDI_DIR_NS dir, MountMotionCommand command)
{
...
bool TemmaMount::MoveWE(INDI_DIR_WE dir, MountMotionCommand command)
{
    char cmd[TEMMA_BUFFER] = {0};
...
#if 0
ln_equ_posn TemmaMount::MountToSky(double ra, double dec)
{
    ...
    if (GetAlignmentDatabase().size() > 1)
    {
        ...
        //  but it's broken for now
            eq.ra=ra*360/24;
            eq.dec=dec;
            TDV=MountDirectionVectorFromEquatorialCoordinates(eq);
        */
        ...
        */
        ...
        eq.ra  = lha;
        eq.dec = dec;
        TDV    = MountDirectionVectorFromLocalHourAngleDeclination(eq);
...
ln_equ_posn TemmaMount::SkyToMount(double ra, double dec)
{
    ln_equ_posn eq { 0, 0 };
    MountDirectionVector TDV;
    double RightAscension, Declination;
    ...
    */
    ...
    if (GetAlignmentDatabase().size() > 1)
    {
        ...
        if (TransformCelestialToMount(ra, dec, 0.0, TDV))
        {
            ...
            EquatorialCoordinatesFromMountDirectionVector(TDV,eq);
            ...
            */
            ...
            double lst;
            LocalHourAngleDeclinationFromMountDirectionVector(TDV, eq);
