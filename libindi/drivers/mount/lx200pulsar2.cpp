...
LX200Pulsar2::LX200Pulsar2() : LX200Generic(), just_started_slewing(false)
{
    ...
    SetMountCapability(MOUNT_CAN_SYNC | MOUNT_CAN_GOTO | MOUNT_CAN_PARK | MOUNT_CAN_ABORT |
                           MOUNT_HAS_TIME | MOUNT_HAS_LOCATION | MOUNT_HAS_PIER_SIDE, 4);
}
...
{
    const bool success = INDI::Mount::Connect();
    if (success)
    {
        if (isParked())
        {
...
{
    // Anything needs to be done besides this? INDI::Mount would call ReadScopeStatus but
...
bool LX200Pulsar2::MoveNS(INDI_DIR_NS dir, MountMotionCommand command)
{
...
bool LX200Pulsar2::MoveWE(INDI_DIR_WE dir, MountMotionCommand command)
{
    const Pulsar2Commands::Direction current_move =
...
bool LX200Pulsar2::UnPark()
{
    ...
    SetParked(false);
    IDMessage(getDeviceName(), "Mount has been unparked.");
    return true;
}
...
void LX200Pulsar2::sendScopeTime()
{
    ...
    if (isDebug())
    {
        IDLog("Mount Local Time: %02d:%02d:%02d\n", ltm.tm_hour, ltm.tm_min, ltm.tm_sec);
        IDLog("Mount TimeT Offset: %s\n", TimeT[1].text);
        IDLog("Mount UTC Time: %s\n", TimeT[0].text);
    }
