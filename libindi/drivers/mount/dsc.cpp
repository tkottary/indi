...
DSC::DSC()
{
    SetMountCapability(MOUNT_CAN_SYNC | MOUNT_HAS_LOCATION, 0);
}
...
bool DSC::Sync(double ra, double dec)
{
    ...
    if (MountTypeS[MOUNT_EQUATORIAL].s == ISS_ON)
        NewEntry.MountDirection = MountDirectionVectorFromLocalHourAngleDeclination(RaDec);
    else
...
ln_equ_posn DSC::MountEquatorialToSky()
{
...
ln_equ_posn DSC::MountHorizontalToSky()
{
    ln_equ_posn eq { 0, 0 };
    MountDirectionVector TDV = MountDirectionVectorFromAltitudeAzimuth(encoderHorizontalCoordinates);
    double RightAscension, Declination;
    ...
    if (!TransformMountToCelestial(TDV, RightAscension, Declination))
    {
        ...
        switch (GetApproximateMountAlignment())
        {
            ...
            case NORTH_CELESTIAL_POLE:
                ...
                AltitudeAzimuthFromMountDirectionVector(RotatedTDV, encoderHorizontalCoordinates);
                break;
