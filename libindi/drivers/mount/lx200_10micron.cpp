...
LX200_10MICRON::LX200_10MICRON() : LX200Generic()
{
    ...
    SetMountCapability( MOUNT_CAN_GOTO | MOUNT_CAN_SYNC | MOUNT_CAN_PARK | MOUNT_CAN_ABORT |
        MOUNT_HAS_TIME | MOUNT_HAS_LOCATION | MOUNT_HAS_PIER_SIDE |
        MOUNT_HAS_TRACK_MODE | MOUNT_CAN_CONTROL_TRACK | MOUNT_HAS_TRACK_RATE );
...
bool LX200_10MICRON::initProperties()
{
    ...
    // TODO initialize properties additional to INDI::Mount
...
// Called by INDI::Mount when connected state changes to add/remove properties
...
// INDI::Mount calls ReadScopeStatus() every POLLMS to check the link to the telescope and update its state and position.
...
bool LX200_10MICRON::ReadScopeStatus()
{
    ...
    setPierSide(toupper(Ginfo.SideOfPier) ? INDI::Mount::PIER_EAST : INDI::Mount::PIER_WEST);
