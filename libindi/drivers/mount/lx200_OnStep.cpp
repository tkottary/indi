...
LX200_OnStep::LX200_OnStep() : LX200Generic()
{
    ...
    SetMountCapability(GetMountCapability() | MOUNT_CAN_CONTROL_TRACK | MOUNT_HAS_PEC | MOUNT_HAS_PIER_SIDE | MOUNT_HAS_TRACK_RATE, 4 );
...
bool LX200_OnStep::GetAlignStatus()
{
    ...
    if (OSAlignProcess && TrackState==MOUNT_SLEWING) OSAlignFlag=true;
    if (OSAlignFlag && TrackState==MOUNT_TRACKING)
    {
        OSAlignFlag=false;
        OSAlignProcess=false;
