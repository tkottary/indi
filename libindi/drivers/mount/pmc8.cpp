...
bool PMC8::Park()
{
    ...
    if (park_pmc8(PortFD))
    {
        ...
        LOG_INFO("Mount parking in progress to motor position (0, 0)");
        return true;
    }
    else
    {
...
bool PMC8::MoveNS(INDI_DIR_NS dir, MountMotionCommand command)
{
...
bool PMC8::MoveWE(INDI_DIR_WE dir, MountMotionCommand command)
{
    if (TrackState == MOUNT_PARKED)
    {
        LOG_ERROR("Please unpark the mount before issuing any motion commands.");
