...
IOptronV3::IOptronV3()
{
    ...
    SetMountCapability(MOUNT_CAN_PARK | MOUNT_CAN_SYNC | MOUNT_CAN_GOTO | MOUNT_CAN_ABORT |
                           MOUNT_HAS_TIME | MOUNT_HAS_LOCATION | MOUNT_HAS_TRACK_MODE |
                           MOUNT_CAN_CONTROL_TRACK | MOUNT_HAS_TRACK_RATE | MOUNT_HAS_PIER_SIDE,
                           9);
}
...
bool IOptronV3::ReadScopeStatus()
{
    ...
    if (rc)
    {
        ...
        /*
        MountTrackMode trackMode = TRACK_SIDEREAL;
...
bool IOptronV3::Park()
{
    ...
    LOGF_INFO("Mount parking in progress to RA: %s DEC: %s", RAStr, DecStr);
...
bool IOptronV3::MoveNS(INDI_DIR_NS dir, MountMotionCommand command)
{
...
bool IOptronV3::MoveWE(INDI_DIR_WE dir, MountMotionCommand command)
{
    if (TrackState == MOUNT_PARKED)
    {
        LOG_ERROR("Please unpark the mount before issuing any motion commands.");
