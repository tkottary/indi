...
LX200Gemini::LX200Gemini()
{
    ...
    SetMountCapability(MOUNT_CAN_PARK | MOUNT_CAN_SYNC | MOUNT_CAN_GOTO | MOUNT_CAN_ABORT |
                               MOUNT_HAS_TIME | MOUNT_HAS_LOCATION | MOUNT_HAS_PIER_SIDE | MOUNT_HAS_TRACK_MODE | MOUNT_CAN_CONTROL_TRACK,
                           4);
}
...
void LX200Gemini::syncSideOfPier()
{
    ...
    setPierSide(response[0] == 'E' ? INDI::Mount::PIER_EAST : INDI::Mount::PIER_WEST);
}
...
void LX200Gemini::setTrackState(INDI::Mount::MountStatus state)
{
    if (TrackState != state)
        TrackState = state;
}
...
void LX200Gemini::updateMovementState()
{
    ...
    switch (movementState)
    {
        case NO_MOVEMENT:
            if (parkingState == PARKED)
                setTrackState(MOUNT_PARKED);
            else
                setTrackState(MOUNT_IDLE);
            break;

        case TRACKING:
        case GUIDING:
            setTrackState(MOUNT_TRACKING);
            break;

        case CENTERING:
        case SLEWING:
            setTrackState(MOUNT_SLEWING);
            break;
