...
{
    bool Ret = INDI::Mount::initProperties();
...
bool SynscanMount::AnalyzeHandset()
{
    ...
    SetMountCapability(caps, SYNSCAN_SLEW_RATES);
...
//        HasFailed = true;
    ...
    */
            ...
            if (strncmp((char *)str, LastParkRead, 18) == 0)
            {
                ...
                if (++StopCount > 2)
                {
                    ...
                    else
                    {
                        ...
                        //IDMessage(getDeviceName(),"Mount is Parked.");
                        SetParked(true);
                    }
                }
                else
    ...
    if (TrackState == MOUNT_SLEWING && MountCode >= 128 && (SlewTargetAz != -1 || SlewTargetAlt != -1))
    {
...
{
    if (TrackState == MOUNT_IDLE || RecoverTrials >= 3)
        return true;
...
bool SynscanMount::MoveNS(INDI_DIR_NS dir, MountMotionCommand command)
{
...
bool SynscanMount::MoveWE(INDI_DIR_WE dir, MountMotionCommand command)
{
    if (command != MOTION_START)
    {
...
{
    bool IsTrackingBeforeSync = (TrackState == MOUNT_TRACKING);
