...
SkywatcherAPIMount::SkywatcherAPIMount()
{
    ...
    SetMountCapability(MOUNT_CAN_PARK | MOUNT_CAN_SYNC | MOUNT_CAN_GOTO | MOUNT_CAN_ABORT |
...
bool SkywatcherAPIMount::initProperties()
{
    ...
    IUFillSwitchVector(&SlewModesSP, SlewModes, 2, getDeviceName(), "MOUNT_MOTION_SLEWMODE", "Slew Mode",
    ...
    IUFillSwitchVector(&SoftPECModesSP, SoftPECModes, 2, getDeviceName(), "MOUNT_MOTION_SOFTPECMODE",
...
void SkywatcherAPIMount::UpdateScopeConfigSwitch()
{
    ...
    for (int i = 1; i < 7; ++i)
    {
        ...
        ISwitch *configSwitch = IUFindSwitch(&ScopeConfigsSP, ("MOUNT_CONFIG"+std::to_string(i)).c_str());
...
bool SkywatcherAPIMount::MoveNS(INDI_DIR_NS dir, MountMotionCommand command)
{
...
bool SkywatcherAPIMount::MoveWE(INDI_DIR_WE dir, MountMotionCommand command)
{
...
bool SkywatcherAPIMount::saveConfigItems(FILE *fp)
{
    ...
    return INDI::Mount::saveConfigItems(fp);
}
...
bool SkywatcherAPIMount::Sync(double ra, double dec)
{
    ...
    DEBUGF(INDI::AlignmentSubsystem::DBG_ALIGNMENT, "New sync point Date %lf RA %lf DEC %lf TDV(x %lf y %lf z %lf)",
           NewEntry.ObservationJulianDate, NewEntry.RightAscension, NewEntry.Declination, NewEntry.MountDirection.x,
           NewEntry.MountDirection.y, NewEntry.MountDirection.z);
...
void SkywatcherAPIMount::TimerHit()
{
    ...
    switch (TrackState)
    {
        ...
        case MOUNT_TRACKING:
        {
            ...
            if (TransformCelestialToMount(CurrentTrackingTarget.ra, CurrentTrackingTarget.dec,
#ifdef USE_INITIAL_JULIAN_DATE
                                              0, TDV))
#else
                                              JulianOffset, TDV))
...
void SkywatcherAPIMount::ConvertGuideCorrection(double delta_ra, double delta_dec, double &delta_alt, double &delta_az)
{
    ...
    MountDirectionVector OldTDV;
    MountDirectionVector NewTDV;

    TransformCelestialToMount(CurrentTrackingTarget.ra, CurrentTrackingTarget.dec, 0.0, OldTDV);
    AltitudeAzimuthFromMountDirectionVector(OldTDV, OldAltAz);
    TransformCelestialToMount(CurrentTrackingTarget.ra+delta_ra, CurrentTrackingTarget.dec+delta_dec, 0.0, NewTDV);
    AltitudeAzimuthFromMountDirectionVector(NewTDV, NewAltAz);
...
void SkywatcherAPIMount::SkywatcherMicrostepsFromMountDirectionVector(
    const MountDirectionVector MountDirectionVector, long &Axis1Microsteps, long &Axis2Microsteps)
{
    // For the time being I assume that all skywathcer mounts share the same encoder conventions
    double Axis1Angle = 0;
    double Axis2Angle = 0;
    SphericalCoordinateFromMountDirectionVector(MountDirectionVector, Axis1Angle,
                                                    MountDirectionVectorSupportFunctions::CLOCKWISE, Axis1Angle,
...
SkywatcherAPIMount::MountDirectionVectorFromSkywatcherMicrosteps(long Axis1Microsteps, long Axis2Microsteps)
{
    // For the time being I assume that all skywathcer mounts share the same encoder conventions
    double Axis1Angle = MicrostepsToRadians(AXIS1, Axis1Microsteps);
    double Axis2Angle = MicrostepsToRadians(AXIS2, Axis2Microsteps);
    return MountDirectionVectorFromSphericalCoordinate(
        Axis1Angle, MountDirectionVectorSupportFunctions::CLOCKWISE, Axis2Angle, FROM_AZIMUTHAL_PLANE);
}
