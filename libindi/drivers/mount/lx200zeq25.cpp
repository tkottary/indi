...
bool LX200ZEQ25::ISNewSwitch(const char *dev, const char *name, ISState *states, char *names[], int n)
{
    if (dev != nullptr && strcmp(dev, getDeviceName()) == 0)
    {
        if (strcmp(HomeSP.name, name) == 0)
        {
            ...
            if (isZEQ25Home())
            {
                LOG_WARN("Mount is already homed.");
                HomeS[0].s = ISS_ON;
                HomeSP.s   = IPS_OK;
...
bool LX200ZEQ25::MoveNS(INDI_DIR_NS dir, MountMotionCommand command)
{
...
bool LX200ZEQ25::MoveWE(INDI_DIR_WE dir, MountMotionCommand command)
{
