...
bool CelestronGPS::updateProperties()
{
    if (isConnected())
    {
        uint32_t cap = MOUNT_CAN_GOTO | MOUNT_CAN_ABORT;
        ...
        if (checkMinVersion(2.3, "updating time and location settings"))
            cap |= MOUNT_HAS_TIME | MOUNT_HAS_LOCATION;


        if (fwInfo.controllerVersion >= 2.3)
            cap |= MOUNT_HAS_TRACK_MODE | MOUNT_CAN_CONTROL_TRACK;
        else
            LOG_WARN("Mount firmware does not support track mode.");

        SetMountCapability(cap, 9);
        ...
        if (fwInfo.controllerVersion >= 2.3)
        {
            ...
            if (driver.get_track_mode(&mode))
            {
                ...
                else
                {
                    ...
                    TrackState = isParked() ? MOUNT_PARKED : MOUNT_IDLE;
                }
            }
            else
...
bool CelestronGPS::MoveNS(INDI_DIR_NS dir, MountMotionCommand command)
{
...
bool CelestronGPS::MoveWE(INDI_DIR_WE dir, MountMotionCommand command)
{
...
bool CelestronGPS::ISNewNumber(const char *dev, const char *name, double values[], char *names[], int n)
{
    ...
    INDI::Mount::ISNewNumber(dev, name, values, names, n);
    return true;
}
...
bool CelestronGPS::UnPark()
{
    ...
    SetParked(false);
    loadConfig(true, "MOUNT_TRACK_MODE");
    return true;
...
bool CelestronGPS::setTrackMode(CELESTRON_TRACK_MODE mode)
{
    ...
    {
        TrackState = (mode == TRACKING_OFF) ? MOUNT_IDLE : MOUNT_TRACKING;
