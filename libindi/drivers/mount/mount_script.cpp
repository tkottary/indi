...
{
    SetMountCapability(MOUNT_CAN_PARK | MOUNT_CAN_SYNC | MOUNT_CAN_GOTO | MOUNT_CAN_ABORT, 4);
}

const char *ScopeScript::getDefaultName()
{
    return (const char *)"Mount Scripting Gateway";
}
...
bool ScopeScript::ISNewText(const char *dev, const char *name, char *texts[], char *names[], int n)
{
    ...
    return Mount::ISNewText(dev, name, texts, names, n);
}
...
bool ScopeScript::MoveNS(INDI_DIR_NS dir, MountMotionCommand command)
{
...
bool ScopeScript::MoveWE(INDI_DIR_WE dir, MountMotionCommand command)
{
    char _rate[] = { (char)('0' + IUFindOnSwitchIndex(&SlewRateSP)), 0 };
    bool status =
