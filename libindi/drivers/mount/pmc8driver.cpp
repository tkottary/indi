...
bool convert_ra_to_motor(double ra, INDI::Mount::MountPierSide sop, int *mcounts)
{
    double motor_angle;
    double hour_angle;
    double lst;
...
//    DEBUGFDEVICE(pmc8_device, INDI::Logger::DBG_DEBUG, "convert_ra_to_motor - ra=%f sop=%d", ra, sop);
    ...
    if (sop == INDI::Mount::PIER_EAST)
        motor_angle = hour_angle - 6;
    else if (sop == INDI::Mount::PIER_WEST)
        motor_angle = hour_angle + 6;
    else
        return false;
...
bool convert_dec_to_motor(double dec, INDI::Mount::MountPierSide sop, int *mcounts)
{
    double motor_angle;
...
INDI::Mount::MountPierSide destSideOfPier(double ra, double dec)
{
    double hour_angle;
    double lst;
    ...
    if (hour_angle < 0.0)
        return INDI::Mount::PIER_WEST;
    else
        return INDI::Mount::PIER_EAST;
}
