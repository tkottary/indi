...
ioptronHC8406::ioptronHC8406()
{
    ...
    SetMountCapability(MOUNT_CAN_PARK | MOUNT_CAN_SYNC | MOUNT_CAN_GOTO |
                      MOUNT_CAN_ABORT | MOUNT_HAS_TIME | MOUNT_HAS_LOCATION |
                      MOUNT_HAS_TRACK_MODE | MOUNT_CAN_CONTROL_TRACK);
...
void ioptronHC8406::sendScopeTime()
{
    ...
    if (isSimulation())
    {
        ...
        IDLog("Mount ISO date and time: %s\n", cdate);
    ...
    if (result != 3)
    {
        LOG_ERROR("Error reading UTC offset from Mount.");
        return;
    }
    ...
    if (result != 3)
    {
        LOG_ERROR("Error reading date from Mount.");
        return;
    }
