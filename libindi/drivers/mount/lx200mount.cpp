...
void LX200Mount::debugTriggered(bool enable)
{
    INDI_UNUSED(enable);
    setLX200Debug(getDeviceName(), DBG_SCOPE);
}

const char *LX200Mount::getDriverName()
{
    return getDefaultName();
}

const char *LX200Mount::getDefaultName()
{
    return (const char *)"Standard LX200 telescope";
}


bool LX200Mount::initProperties()
{
    ...
    IUFillSwitchVector(&SlewRateSP, SlewRateS, 4, getDeviceName(), "MOUNT_SLEW_RATE", "Slew Rate", MOTION_TAB, IP_RW, ISR_1OFMANY, 0, IPS_IDLE);
#endif
    ...
    IUFillSwitchVector(&TrackModeSP, TrackModeS, 4, getDeviceName(), "MOUNT_TRACK_MODE", "Track Mode", MOTION_TAB,
...
void LX200Mount::ISGetProperties(const char *dev)
{
    if (dev != nullptr && strcmp(dev, getDeviceName()) != 0)
        return;

    INDI::Mount::ISGetProperties(dev);
...
bool LX200Mount::updateProperties()
{
    INDI::Mount::updateProperties();
...
bool LX200Mount::checkConnection()
{
    if (isSimulation())
        return true;
...
bool LX200Mount::Handshake()
{
    return checkConnection();
}

bool LX200Mount::isSlewComplete()
{
    return (::isSlewComplete(PortFD) == 1);
}

bool LX200Mount::ReadScopeStatus()
{
    if (!isConnected())
        return false;
    ...
    else if (TrackState == MOUNT_PARKING)
    {
        if (isSlewComplete())
        {
            SetParked(true);
...
bool LX200Mount::Goto(double ra, double dec)
{
...
bool LX200Mount::Sync(double ra, double dec)
{
...
bool LX200Mount::MoveNS(INDI_DIR_NS dir, MountMotionCommand command)
{
...
bool LX200Mount::MoveWE(INDI_DIR_WE dir, MountMotionCommand command)
{
...
bool LX200Mount::setLocalDate(uint8_t days, uint8_t months, uint16_t years)
{
    return (setCalenderDate(PortFD, days, months, years) == 0);
}

bool LX200Mount::setLocalTime24(uint8_t hour, uint8_t minute, uint8_t second)
{
    return (setLocalTime(PortFD, hour, minute, second) == 0);
}

bool LX200Mount::setUTCOffset(double offset)
{
    return (::setUTCOffset(PortFD, (offset * -1.0)) == 0);
}

bool LX200Mount::updateTime(ln_date *utc, double utc_offset)
{
...
bool LX200Mount::updateLocation(double latitude, double longitude, double elevation)
{
...
bool LX200Mount::ISNewText(const char *dev, const char *name, char *texts[], char *names[], int n)
{
    if (dev != nullptr && strcmp(dev, getDeviceName()) == 0)
    {
...
bool LX200Mount::ISNewNumber(const char *dev, const char *name, double values[], char *names[], int n)
{
    if (dev != nullptr && strcmp(dev, getDeviceName()) == 0)
    {
        // Update Frequency
...
bool LX200Mount::ISNewSwitch(const char *dev, const char *name, ISState *states, char *names[], int n)
{
    int index = 0;
    ...
    if (dev != nullptr && strcmp(dev, getDeviceName()) == 0)
    {
        ...
        if (!strcmp(name, SiteSP.name))
        {
            ...
            if (GetMountCapability() & MOUNT_HAS_LOCATION)
                sendScopeLocation();
        ...
        if (!strcmp(name, FocusMotionSP.name))
        {
            ...
            if (FocusTimerN[0].value > 0)
            {
                ...
                IEAddTimer(50, LX200Mount::updateFocusHelper, this);
            }
...
bool LX200Mount::SetTrackMode(uint8_t mode)
{
    if (isSimulation())
        return true;
...
bool LX200Mount::SetSlewRate(int index)
{
...
void LX200Mount::updateFocusHelper(void *p)
{
    ((LX200Mount *)p)->updateFocusTimer();
}

void LX200Mount::updateFocusTimer()
{
...
void LX200Mount::getBasicData()
{
    if (!isSimulation())
    {
        ...
        if ( (GetMountCapability() & MOUNT_HAS_TIME) && timeFormat == -1)
        {
    ...
    if (sendLocationOnStartup && (GetMountCapability() & MOUNT_HAS_LOCATION))
        sendScopeLocation();
    if (sendTimeOnStartup && (GetMountCapability() & MOUNT_HAS_TIME))
        sendScopeTime();
}

void LX200Mount::slewError(int slewCode)
{
    if (slewCode == 1)
...
bool LX200Mount::getLocalTime(char *timeString)
{
    if (isSimulation())
    {
...
bool LX200Mount::getLocalDate(char *dateString)
{
    if (isSimulation())
    {
        time_t now = time (NULL);
...
bool LX200Mount::getUTFOffset(double *offset)
{
    if (isSimulation())
    {
        *offset = 3;
...
bool LX200Mount::sendScopeLocation()
{
    int dd = 0, mm = 0;
...
IPState LX200Mount::GuideNorth(float ms)
{
    int use_pulse_cmd;
...
IPState LX200Mount::GuideSouth(float ms)
{
    int use_pulse_cmd;
...
IPState LX200Mount::GuideEast(float ms)
{
    int use_pulse_cmd;
...
IPState LX200Mount::GuideWest(float ms)
{
    int use_pulse_cmd;
...
int LX200Mount::SendPulseCmd(int direction, int duration_msec)
{
    return ::SendPulseCmd(PortFD, direction, duration_msec);
}

void LX200Mount::guideTimeoutHelper(void * p)
{
    ((LX200Mount *)p)->guideTimeout();
}

void LX200Mount::guideTimeout()
{
    int use_pulse_cmd;
...
bool LX200Mount::saveConfigItems(FILE *fp)
{
    INDI::Mount::saveConfigItems(fp);
