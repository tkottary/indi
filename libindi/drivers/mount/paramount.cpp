...
Paramount::Paramount()
{
    ...
    SetMountCapability(MOUNT_CAN_PARK | MOUNT_CAN_SYNC | MOUNT_CAN_GOTO | MOUNT_CAN_ABORT |
                               MOUNT_HAS_TIME | MOUNT_HAS_LOCATION | MOUNT_HAS_TRACK_MODE | MOUNT_HAS_TRACK_RATE | MOUNT_CAN_CONTROL_TRACK,
                           9);
    setMountConnection(CONNECTION_TCP);
}
...
bool Paramount::initProperties()
{
    ...
    IUFillSwitchVector(&TrackModeSP, TrackModeS, 4, getDeviceName(), "MOUNT_TRACK_MODE", "Track Mode",
    ...
    IUFillNumberVector(&TrackRateNP, TrackRateN, 2, getDeviceName(), "MOUNT_TRACK_RATE", "Track Rates",
...
bool Paramount::Handshake()
{
    ...
    int isMountConnected = -1;

    std::regex rgx(R"((\d+)\|(.+)\. Error = (\d+)\.)");
    std::smatch match;
    std::string input(pRES);
    if (std::regex_search(input, match, rgx))
        isMountConnected = atoi(match.str(1).c_str());

    if (isMountConnected <= 0)
    {
...
bool Paramount::MoveNS(INDI_DIR_NS dir, MountMotionCommand command)
{
...
bool Paramount::MoveWE(INDI_DIR_WE dir, MountMotionCommand command)
{
    if (TrackState == MOUNT_PARKED)
    {
        LOG_ERROR("Please unpark the mount before issuing any motion commands.");
...
IPState Paramount::GuideNorth(float ms)
{
    ...
    char pCMD[MAXRBUF]={0};
    snprintf(pCMD, MAXRBUF, "sky6DirectGuide.MoveMount(%g, %g);", 0., dDec);
...
IPState Paramount::GuideEast(float ms)
{
    ...
    snprintf(pCMD, MAXRBUF, "sky6DirectGuide.MoveMount(%g, %g);", dRA, 0.);
