...
IEQPro::IEQPro()
{
    ...
    SetMountCapability(MOUNT_CAN_PARK | MOUNT_CAN_SYNC | MOUNT_CAN_GOTO | MOUNT_CAN_ABORT |
                           MOUNT_HAS_TIME | MOUNT_HAS_LOCATION | MOUNT_HAS_TRACK_MODE | MOUNT_CAN_CONTROL_TRACK | MOUNT_HAS_TRACK_RATE,
                           9);
}
...
bool IEQPro::MoveNS(INDI_DIR_NS dir, MountMotionCommand command)
{
...
bool IEQPro::MoveWE(INDI_DIR_WE dir, MountMotionCommand command)
{
    if (TrackState == MOUNT_PARKED)
    {
        LOG_ERROR("Please unpark the mount before issuing any motion commands.");
