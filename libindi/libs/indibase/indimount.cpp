...
bool Mount::initProperties()
{
    ...
    IUFillSwitchVector(&ParkOptionSP, ParkOptionS, 3, getDeviceName(), "MOUNT_PARK_OPTION", "Park Options",
    ...
    IUFillSwitchVector(&PierSideSP, PierSideS, 2, getDeviceName(), "MOUNT_PIER_SIDE", "Pier Side", MAIN_CONTROL_TAB,
    ...
    IUFillSwitchVector(&TrackModeSP, TrackModeS, 0, getDeviceName(), "MOUNT_TRACK_MODE", "Track Mode", MAIN_CONTROL_TAB,
                       IP_RW, ISR_1OFMANY, 0, IPS_IDLE);

    // Track State
    IUFillSwitch(&TrackStateS[TRACK_ON], "TRACK_ON", "On", ISS_OFF);
    IUFillSwitch(&TrackStateS[TRACK_OFF], "TRACK_OFF", "Off", ISS_ON);
    IUFillSwitchVector(&TrackStateSP, TrackStateS, 2, getDeviceName(), "MOUNT_TRACK_STATE", "Tracking", MAIN_CONTROL_TAB, IP_RW, ISR_1OFMANY, 0,
                       IPS_IDLE);
    ...
    IUFillNumberVector(&TrackRateNP, TrackRateN, 2, getDeviceName(), "MOUNT_TRACK_RATE", "Track Rates", MAIN_CONTROL_TAB,
                       IP_RW, 60, IPS_IDLE);
    ...
    if (nSlewRate >= 4)
        IUFillSwitchVector(&SlewRateSP, SlewRateS, nSlewRate, getDeviceName(), "MOUNT_SLEW_RATE", "Slew Rate",
                           MOTION_TAB, IP_RW, ISR_1OFMANY, 0, IPS_IDLE);

    IUFillSwitch(&ParkS[0], "PARK", "Park", ISS_OFF);
    IUFillSwitch(&ParkS[1], "UNPARK", "UnPark", ISS_OFF);
    IUFillSwitchVector(&ParkSP, ParkS, 2, getDeviceName(), "MOUNT_PARK", "Parking", MAIN_CONTROL_TAB, IP_RW,
                       ISR_1OFMANY, 60, IPS_IDLE);

    IUFillSwitch(&AbortS[0], "ABORT", "Abort", ISS_OFF);
    IUFillSwitchVector(&AbortSP, AbortS, 1, getDeviceName(), "MOUNT_ABORT_MOTION", "Abort Motion", MAIN_CONTROL_TAB,
    ...
    IUFillSwitchVector(&MovementNSSP, MovementNSS, 2, getDeviceName(), "MOUNT_MOTION_NS", "Motion N/S", MOTION_TAB,
                       IP_RW, ISR_ATMOST1, 60, IPS_IDLE);
    ...
    IUFillSwitchVector(&MovementWESP, MovementWES, 2, getDeviceName(), "MOUNT_MOTION_WE", "Motion W/E", MOTION_TAB,
                       IP_RW, ISR_ATMOST1, 60, IPS_IDLE);

    IUFillNumber(&ScopeParametersN[0], "MOUNT_APERTURE", "Aperture (mm)", "%g", 10, 5000, 0, 0.0);
    IUFillNumber(&ScopeParametersN[1], "MOUNT_FOCAL_LENGTH", "Focal Length (mm)", "%g", 10, 10000, 0, 0.0);
    ...
    IUFillNumberVector(&ScopeParametersNP, ScopeParametersN, 4, getDeviceName(), "MOUNT_INFO", "Scope Properties",
                       OPTIONS_TAB, IP_RW, 60, IPS_OK);

    // Scope config name
    IUFillText(&ScopeConfigNameT[0], "MOUNT_CONFIG_NAME", "Config Name", "");
    IUFillTextVector(&ScopeConfigNameTP, ScopeConfigNameT, 1, getDeviceName(), "MOUNT_CONFIG_NAME", "Scope Name",
                     OPTIONS_TAB, IP_RW, 60, IPS_OK);

    // Switch for aperture/focal length configs
    IUFillSwitch(&ScopeConfigs[MOUNT_CONFIG1], "MOUNT_CONFIG1", "Config #1", ISS_ON);
    IUFillSwitch(&ScopeConfigs[MOUNT_CONFIG2], "MOUNT_CONFIG2", "Config #2", ISS_OFF);
    IUFillSwitch(&ScopeConfigs[MOUNT_CONFIG3], "MOUNT_CONFIG3", "Config #3", ISS_OFF);
    IUFillSwitch(&ScopeConfigs[MOUNT_CONFIG4], "MOUNT_CONFIG4", "Config #4", ISS_OFF);
    IUFillSwitch(&ScopeConfigs[MOUNT_CONFIG5], "MOUNT_CONFIG5", "Config #5", ISS_OFF);
    IUFillSwitch(&ScopeConfigs[MOUNT_CONFIG6], "MOUNT_CONFIG6", "Config #6", ISS_OFF);
    IUFillSwitchVector(&ScopeConfigsSP, ScopeConfigs, 6, getDeviceName(), "APPLY_MOUNT_CONFIG", "Scope Configs",
    ...
    if (mountConnection & CONNECTION_SERIAL)
    {
    ...
    if (mountConnection & CONNECTION_TCP)
    {
...
void Mount::ISGetProperties(const char *dev)
{
    ...
    {
        loadConfig(true, "MOUNT_INFO");
        loadConfig(true, "MOUNT_CONFIG_NAME");
    }
...
bool Mount::ISSnoopDevice(XMLEle *root)
{
...
void Mount::triggerSnoop(const char *driverName, const char *snoopedProp)
{
    DEBUGF(Logger::DBG_DEBUG, "Active Snoop, driver: %s, property: %s", driverName, snoopedProp);
    IDSnoopDevice(driverName, snoopedProp);
}

uint8_t Mount::getMountConnection() const
{
    return mountConnection;
}

void Mount::setMountConnection(const uint8_t &value)
{
    ...
    mountConnection = value;
}

bool Mount::saveConfigItems(FILE *fp)
{
...
void Mount::NewRaDec(double ra, double dec)
{
    switch (TrackState)
    {
        case MOUNT_PARKED:
    ...
    if (TrackState != MOUNT_TRACKING && CanControlTrack() && TrackStateS[TRACK_ON].s == ISS_ON)
    {
        TrackStateSP.s = IPS_IDLE;
        TrackStateS[TRACK_ON].s = ISS_OFF;
        TrackStateS[TRACK_OFF].s = ISS_ON;
        IDSetSwitch(&TrackStateSP, nullptr);
    } else if (TrackState == MOUNT_TRACKING && CanControlTrack() && TrackStateS[TRACK_OFF].s == ISS_ON)
    {
...
bool Mount::MoveNS(INDI_DIR_NS dir, MountMotionCommand command)
{
    INDI_UNUSED(dir);
    INDI_UNUSED(command);
...
bool Mount::MoveWE(INDI_DIR_WE dir, MountMotionCommand command)
{
    INDI_UNUSED(dir);
    INDI_UNUSED(command);
...
***************************************************************************************/
bool Mount::ISNewText(const char *dev, const char *name, char *texts[], char *names[], int n)
{
    ...
    if (dev != nullptr && strcmp(dev, getDeviceName()) == 0)
    {
        ...
        if (name && std::string(name) == "MOUNT_CONFIG_NAME")
        {
...
**
***************************************************************************************/
bool Mount::ISNewNumber(const char *dev, const char *name, double values[], char *names[], int n)
{
    //  first check if it's for our device
    if (dev != nullptr && strcmp(dev, getDeviceName()) == 0)
    {
        ...
        if (strcmp(name, TrackRateNP.name) == 0)
        {
            ...
            if (TrackState == MOUNT_TRACKING && !strcmp(IUFindOnSwitch(&TrackModeSP)->name, "TRACK_CUSTOM"))
            {
...
**
***************************************************************************************/
bool Mount::ISNewSwitch(const char *dev, const char *name, ISState *states, char *names[], int n)
{
    if (dev != nullptr && strcmp(dev, getDeviceName()) == 0)
    {
        //  This one is for us
        ...
        if (!strcmp(name, ParkSP.name))
        {
            ...
            if (toPark == false && TrackState != MOUNT_PARKED)
            {
                IUResetSwitch(&ParkSP);
                ParkS[1].s = ISS_ON;
                ParkSP.s   = IPS_IDLE;
                LOG_INFO("Mount already unparked.");
                IsParked = false;
                IDSetSwitch(&ParkSP, nullptr);
                return true;
            }
            ...
            if (toPark && TrackState == MOUNT_PARKED)
            {
                IUResetSwitch(&ParkSP);
                ParkS[0].s = ISS_ON;
                ParkSP.s   = IPS_IDLE;
                DEBUG(Logger::DBG_SESSION, "Mount already parked.");
                IDSetSwitch(&ParkSP, nullptr);
                return true;
            }
        ...
        if (!strcmp(name, MovementNSSP.name))
        {
            ...
            {
                if (TrackState != MOUNT_SLEWING && TrackState != MOUNT_PARKING)
                    RememberTrackState = TrackState;
        ...
        if (!strcmp(name, TrackModeSP.name))
        {
            ...
            if (prevIndex == currIndex || TrackState == MOUNT_IDLE)
            {
            ...
            if (TrackState == MOUNT_PARKED)
            {
                DEBUG(Logger::DBG_WARNING, "Mount is Parked, Unpark before changing track mode.");
                return false;
            }
        ...
        if (!strcmp(name, TrackStateSP.name))
        {
            ...
            if (TrackState == MOUNT_PARKED)
            {
                DEBUG(Logger::DBG_WARNING, "Mount is Parked, Unpark before tracking.");
                return false;
            }
            ...
            if (rc)
            {
                TrackState = (targetState == TRACK_ON) ? MOUNT_TRACKING : MOUNT_IDLE;
        ...
        if (!strcmp(name, ParkOptionSP.name))
        {
            ...
            if ((TrackState != MOUNT_IDLE && TrackState != MOUNT_TRACKING) || MovementNSSP.s == IPS_BUSY ||
                MovementWESP.s == IPS_BUSY)
            {
        ...
        // Scope Apply Config
        ///////////////////////////////////
        if (name && std::string(name) == "APPLY_MOUNT_CONFIG")
        {
...
bool Mount::callHandshake()
{
    if (mountConnection > 0)
    {
...
bool Mount::SetTrackMode(uint8_t mode)
{
    INDI_UNUSED(mode);
...
bool Mount::SetTrackRate(double raRate, double deRate)
{
    INDI_UNUSED(raRate);
    INDI_UNUSED(deRate);
...
bool Mount::SetTrackEnabled(bool enabled)
{
    INDI_UNUSED(enabled);
...
int Mount::AddTrackMode(const char *name, const char *label, bool isDefault)
{
...
bool Mount::SetCurrentPark()
{
    DEBUG(Logger::DBG_WARNING, "Parking is not supported.");
    return false;
}

bool Mount::SetDefaultPark()
{
    DEBUG(Logger::DBG_WARNING, "Parking is not supported.");
    return false;
}

bool Mount::processTimeInfo(const char *utc, const char *offset)
{
...
bool Mount::processLocationInfo(double latitude, double longitude, double elevation)
{
...
bool Mount::updateTime(ln_date *utc, double utc_offset)
{
    INDI_UNUSED(utc);
...
bool Mount::updateLocation(double latitude, double longitude, double elevation)
{
...
bool Mount::SetParkPosition(double Axis1Value, double Axis2Value)
{
...
void Mount::SetMountCapability(uint32_t cap, uint8_t slewRateCount)
{
    capability = cap;
...
void Mount::SetParkDataType(MountParkData type)
{
    parkDataType = type;
    ...
    if (parkDataType != PARK_NONE)
    {
        ...
        IUFillNumberVector(&ParkPositionNP, ParkPositionN, 2, getDeviceName(), "MOUNT_PARK_POSITION",
                           "Park Position", SITE_TAB, IP_RW, 60, IPS_IDLE);
    }
}

void Mount::SetParked(bool isparked)
{
    IsParked = isparked;
    IUResetSwitch(&ParkSP);
...
double Mount::GetAxis1Park() const
{
    return Axis1ParkPosition;
}
double Mount::GetAxis1ParkDefault() const
{
    return Axis1DefaultParkPosition;
}
double Mount::GetAxis2Park() const
{
    return Axis2ParkPosition;
}
double Mount::GetAxis2ParkDefault() const
{
    return Axis2DefaultParkPosition;
}

void Mount::SetAxis1Park(double value)
{
...
void Mount::SetAxis1ParkDefault(double value)
{
    Axis1DefaultParkPosition = value;
}

void Mount::SetAxis2Park(double value)
{
...
void Mount::SetAxis2ParkDefault(double value)
{
    Axis2DefaultParkPosition = value;
}

bool Mount::isLocked() const
{
...
void Mount::processButton(const char *button_n, ISState state)
{
    //ignore OFF
    if (state == ISS_OFF)
        return;
    ...
    {
        ISwitchVectorProperty *trackSW = getSwitch("MOUNT_TRACK_MODE");
        ...
        {
            // Invoke parent processing so that Mount takes care of abort cross-check
...
void Mount::processJoystick(const char *joystick_n, double mag, double angle)
{
    if (!strcmp(joystick_n, "MOTIONDIR"))
    {
        if ((TrackState == MOUNT_PARKING) || (TrackState == MOUNT_PARKED))
        {
...
void Mount::processNSWE(double mag, double angle)
{
    if (mag < 0.5)
    {
...
void Mount::processSlewPresets(double mag, double angle)
{
...
void Mount::joystickHelper(const char *joystick_n, double mag, double angle, void *context)
{
    static_cast<Mount *>(context)->processJoystick(joystick_n, mag, angle);
}

void Mount::buttonHelper(const char *button_n, ISState state, void *context)
{
    static_cast<Mount *>(context)->processButton(button_n, state);
}

void Mount::setPierSide(MountPierSide side)
{
...
void Mount::setPECState(MountPECState state)
{
...
bool Mount::LoadScopeConfig()
{
    ...
    if (IUFindNumber(&ScopeParametersNP, "MOUNT_FOCAL_LENGTH"))
    {
        IUFindNumber(&ScopeParametersNP, "MOUNT_FOCAL_LENGTH")->value = ScopeFoc;
    }
    if (IUFindNumber(&ScopeParametersNP, "MOUNT_APERTURE"))
    {
        IUFindNumber(&ScopeParametersNP, "MOUNT_APERTURE")->value = ScopeAp;
    }
    ...
    if (IUFindText(&ScopeConfigNameTP, "MOUNT_CONFIG_NAME"))
    {
        IUSaveText(IUFindText(&ScopeConfigNameTP, "MOUNT_CONFIG_NAME"), ConfigName.c_str());
    }
    ScopeParametersNP.s = IPS_OK;
    IDSetNumber(&ScopeParametersNP, nullptr);
    ScopeConfigNameTP.s = IPS_OK;
...
bool Mount::HasDefaultScopeConfig()
{
    if (!CheckFile(ScopeConfigFileName, false))
    {
        return false;
...
bool Mount::UpdateScopeConfig()
{
    ...
    if (IUFindNumber(&ScopeParametersNP, "MOUNT_FOCAL_LENGTH"))
    {
        ScopeFoc = IUFindNumber(&ScopeParametersNP, "MOUNT_FOCAL_LENGTH")->value;
    }
    if (IUFindNumber(&ScopeParametersNP, "MOUNT_APERTURE"))
    {
        ScopeAp = IUFindNumber(&ScopeParametersNP, "MOUNT_APERTURE")->value;
    }
    if (IUFindNumber(&ScopeParametersNP, "GUIDER_FOCAL_LENGTH"))
    {
    ...
    if (IUFindText(&ScopeConfigNameTP, "MOUNT_CONFIG_NAME") &&
        IUFindText(&ScopeConfigNameTP, "MOUNT_CONFIG_NAME")->text)
    {
        ConfigName = IUFindText(&ScopeConfigNameTP, "MOUNT_CONFIG_NAME")->text;
    }
...
std::string Mount::GetHomeDirectory() const
{
...
int Mount::GetScopeConfigIndex() const
{
    if (IUFindSwitch(&ScopeConfigsSP, "MOUNT_CONFIG1") && IUFindSwitch(&ScopeConfigsSP, "MOUNT_CONFIG1")->s == ISS_ON)
    {
        return 1;
    }
    if (IUFindSwitch(&ScopeConfigsSP, "MOUNT_CONFIG2") && IUFindSwitch(&ScopeConfigsSP, "MOUNT_CONFIG2")->s == ISS_ON)
    {
        return 2;
    }
    if (IUFindSwitch(&ScopeConfigsSP, "MOUNT_CONFIG3") && IUFindSwitch(&ScopeConfigsSP, "MOUNT_CONFIG3")->s == ISS_ON)
    {
        return 3;
    }
    if (IUFindSwitch(&ScopeConfigsSP, "MOUNT_CONFIG4") && IUFindSwitch(&ScopeConfigsSP, "MOUNT_CONFIG4")->s == ISS_ON)
    {
        return 4;
    }
    if (IUFindSwitch(&ScopeConfigsSP, "MOUNT_CONFIG5") && IUFindSwitch(&ScopeConfigsSP, "MOUNT_CONFIG5")->s == ISS_ON)
    {
        return 5;
    }
    if (IUFindSwitch(&ScopeConfigsSP, "MOUNT_CONFIG6") && IUFindSwitch(&ScopeConfigsSP, "MOUNT_CONFIG6")->s == ISS_ON)
    {
        return 6;
    }
    return 0;
...
bool Mount::CheckFile(const std::string &file_name, bool writable) const
{
